...
let myflavors = ["chocolate", "vanilla", "coconut"];
removeFlavorByName(myflavors, "vanilla");
removeFlavorByName(myflavors, "orange");
...
function getAverageWordLength(array){
    let newArray = [];
    for(let i= 0; i<array.length; i++) {
        newArray.push( array[i].length/array.length)
    }
  return newArray
}
...
const allFlavour = originalFlavors.concat(newFlavors,seasonalFlavors,regionalFlavors);
function getRandomFlavors(array) {
    let random = Math.round(Math.random() * (allFlavour.length - 1));
    const newArray = [];
    for(let i=0; i<=30; i++) {
        newArray.push(array[i]);
    }
...
console.log(getRandomFlavors(allFlavour));
